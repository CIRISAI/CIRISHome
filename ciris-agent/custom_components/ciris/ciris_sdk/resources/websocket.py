...
class ChannelFilter(BaseModel):
    ...
    level: Optional[str] = Field(
        None, description="Minimum log level (DEBUG, INFO, WARNING, ERROR)"
    )
                    ...
                    try:
                        ...
                        try:
                            ...
                            try:
                                ...
                                self._message_queue.put_nowait(ws_message)
                                logger.warning(
                                    "Message buffer full, dropped oldest message"
                                )
...
class ReconnectionPolicy(BaseModel):
    ...
    initial_interval: float = Field(
        1.0, description="Initial reconnection interval in seconds"
    )
    max_interval: float = Field(60.0, description="Maximum reconnection interval")
    max_attempts: Optional[int] = Field(
        None, description="Maximum reconnection attempts (None=infinite)"
    )
...
class BackpressurePolicy(BaseModel):
    ...
    drop_policy: str = Field(
        "oldest", description="Which messages to drop when full (oldest/newest)"
    )
    warn_threshold: float = Field(
        0.8, description="Warn when buffer is this full (0.0-1.0)"
    )
