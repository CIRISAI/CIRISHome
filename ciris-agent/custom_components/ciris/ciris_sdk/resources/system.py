...
class SystemHealthResponse(BaseModel):
    ...
    services: Dict[str, Dict[str, int]] = Field(
        ..., description="Service health summary"
    )
    initialization_complete: bool = Field(
        ..., description="Whether system initialization is complete"
    )
    cognitive_state: Optional[str] = Field(
        None, description="Current cognitive state if available"
    )
...
class SystemTimeResponse(BaseModel):
    ...
    utc_offset: Optional[str] = Field(
        default="+00:00", description="Current UTC offset (e.g., '-05:00')"
    )
    is_dst: Optional[bool] = Field(
        default=False, description="Whether daylight saving time is active"
    )
...
class ResourceSnapshot(BaseModel):
    ...
    timestamp: Optional[datetime] = Field(
        default=None, description="When snapshot was taken"
    )
...
class ResourceBudget(BaseModel):
    ...
    max_memory_mb: Optional[float] = Field(
        default=None, description="Maximum memory in MB"
    )
    max_cpu_percent: Optional[float] = Field(
        default=None, description="Maximum CPU percentage"
    )
    max_open_files: Optional[int] = Field(
        default=None, description="Maximum open files"
    )
...
class ResourceUsageResponse(BaseModel):
    ...
    critical: List[str] = Field(
        default_factory=list, description="Critical resource issues"
    )
...
class ServiceStatus(BaseModel):
    ...
    uptime_seconds: Optional[float] = Field(
        None, description="Service uptime if tracked"
    )
    metrics: ServiceMetrics = Field(
        default_factory=ServiceMetrics, description="Service-specific metrics"
    )
...
class SystemResource:
    ...
    async def runtime_control(
        self, action: str, reason: Optional[str] = None
        ...
        result = await self._transport.request(
            "POST", f"/v1/system/runtime/{action}", json=body
        )
    ...
    async def shutdown(
        self, reason: str, grace_period_seconds: int = 30, force: bool = False
    ) -> ShutdownResponse:
        """
