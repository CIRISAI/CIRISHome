...
class MemoryQueryRequest(BaseModel):
    ...
    tags: Optional[List[str]] = Field(
        None, description="Filter by tags (AND operation)"
    )
    since: Optional[datetime] = Field(
        None, description="Filter by creation time (after)"
    )
    until: Optional[datetime] = Field(
        None, description="Filter by creation time (before)"
    )
    related_to: Optional[str] = Field(
        None, description="Find nodes related to this node ID"
    )
...
class MemoryQueryResponse(BaseModel):
    ...
    total_matches: Optional[int] = Field(
        None, description="Total matches (expensive, only if requested)"
    )
...
class TimelineResponse(BaseModel):
    ...
    buckets: Union[List[Dict[str, Any]], Dict[str, int]] = Field(
        ..., description="Time bucket counts"
    )
...
class MemoryResource:
    ...
    **WARNING**: This SDK is for the v1 API which is in pre-beta stage.
    ...
    async def store(
        self, node: Union[Dict[str, Any], GraphNode]
    ) -> MemoryStoreResponse:
        """
        ...
        if isinstance(result, dict):
            ...
            if api_result and "status" in api_result:
                return MemoryStoreResponse(
                    success=api_result.get("status", "error").lower()
                    in ["ok", "success"],
    ...
    async def query(
        ...
        query: Optional[
            str
        ] = None,  # Allow positional query parameter for backward compatibility
        *,
    ...
    async def get_timeline(
        ...
        result = await self._transport.request(
            "GET", "/v1/memory/timeline", params=params
        )
    ...
    async def query_recent(
        self, hours: int = 24, type: Optional[str] = None
    ) -> List[GraphNode]:
        """
        Query recent nodes.
