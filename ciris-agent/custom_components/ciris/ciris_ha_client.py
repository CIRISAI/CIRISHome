...
from .ciris_sdk.exceptions import CIRISError, CIRISTimeoutError  # noqa: F401

__all__ = ["CIRISClient", "CIRISError", "CIRISTimeoutError"]
...
class CIRISClient(SDKCIRISClient):
    ...
    async def __aenter__(self):
        ...
        _LOGGER.info(
            "Creating httpx client with base URL: %s", self._transport.base_url
        )
        self._transport._client = httpx.AsyncClient(
            timeout=httpx.Timeout(self._transport.timeout),
            verify=False,  # nosec B501 - Intentional for local network
