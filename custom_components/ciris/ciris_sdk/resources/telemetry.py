...
class TelemetryResourcesHistory(BaseModel):
    ...
    period: Optional[Union[str, Dict[str, Any]]] = Field(
        None, description="Time period"
    )
    cpu: Union[List[Dict[str, Any]], Dict[str, Any]] = Field(
        ..., description="CPU history"
    )
    memory: Union[List[Dict[str, Any]], Dict[str, Any]] = Field(
...
class TelemetryResource:
    ...
    async def get_traces(
        ...
        data = await self._transport.request(
            "GET", "/v1/telemetry/traces", params=params
        )
        return data
    ...
    async def query(
        ...
        Requires ADMIN role.
        """
        payload = {"query_type": query_type, "filters": filters or {}, "limit": limit}
        ...
        data = await self._transport.request(
            "POST", "/v1/telemetry/query", json=payload
        )
        return data
    ...
    async def metric_detail(self, metric_name: str) -> TelemetryMetricDetail:
        ...
        data = await self._transport.request(
            "GET", f"/v1/telemetry/metrics/{metric_name}"
        )
    ...
    async def resources_history(self, hours: int = 24) -> TelemetryResourcesHistory:
        ...
        data = await self._transport.request(
            "GET", "/v1/telemetry/resources/history", params=params
        )
