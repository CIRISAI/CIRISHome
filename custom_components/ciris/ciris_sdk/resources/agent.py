...
class AgentResource:
    ...
    async def interact(
        ...
        result = await self._transport.request(
            "POST", "/v1/agent/interact", json=request_data
        )

        return InteractResponse(**result)

    async def get_history(
        self, limit: int = 50, before: Optional[datetime] = None
    ) -> ConversationHistory:
        ...
        result = await self._transport.request(
            "GET", "/v1/agent/history", params=params
        )
    ...
    async def list_channels(self) -> Dict[str, Any]:
        ...
        return {
            "channels": [
                {"id": "api_default", "name": "Default API Channel", "active": True}
            ]
        }
