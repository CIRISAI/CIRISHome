...
class WiseAuthorityResource:
    ...
    async def resolve_deferral(
        ...
        data = await self._transport.request(
            "POST", f"/v1/wa/deferrals/{deferral_id}/resolve", json=payload
        )
        return data
    ...
    async def approve_deferral(
        self, deferral_id: str, guidance: Optional[str] = None
    ) -> Dict[str, Any]:
        """Approve a deferral with optional guidance."""
        return await self.resolve_deferral(
            deferral_id=deferral_id, resolution="approved", guidance=guidance
        )

    async def reject_deferral(self, deferral_id: str, reasoning: str) -> Dict[str, Any]:
        """Reject a deferral with reasoning."""
        return await self.resolve_deferral(
            deferral_id=deferral_id, resolution="rejected", reasoning=reasoning
        )

    async def modify_deferral(
        self, deferral_id: str, guidance: str, reasoning: Optional[str] = None
    ) -> Dict[str, Any]:
