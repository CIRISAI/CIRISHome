...
class WASignedCommand(BaseModel):
    ...
    command_type: EmergencyCommandType = Field(
    ...
    target_agent_id: Optional[str] = Field(
        None, description="Specific agent or None for all"
    )
    target_tree_path: Optional[List[str]] = Field(
        None, description="WA tree path for targeting"
    )
    ...
    parent_command_id: Optional[str] = Field(
        None, description="Parent command if relayed"
    )
    relay_chain: List[str] = Field(
        default_factory=list, description="WA IDs in relay chain"
    )
...
class EmergencyResource:
    ...
    **CRITICAL**: The emergency shutdown endpoint bypasses normal authentication
    ...
    async def shutdown(
        self,
        reason: str,
        private_key: "Ed25519PrivateKey",  # pragma: allowlist secret
        wa_id: str,
        wa_public_key: str,
        ...
        result = await self._transport.request(
            "POST", "/emergency/shutdown", json=command.dict()
        )

        return EmergencyShutdownResponse(**result)

    def _sign_command(
        self, command: WASignedCommand, private_key: Ed25519PrivateKey
    ) -> str:
        """
    ...
    def verify_signature(
        self, command: WASignedCommand, public_key_bytes: bytes
    ) -> bool:
        """
